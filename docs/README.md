# CodingClaw Documentation Map

## Canonical Entry

Start with these documents in order:

1. [SYSTEM_BLUEPRINT.md](SYSTEM_BLUEPRINT.md)
2. [ARCHITECTURE_OVERVIEW.md](ARCHITECTURE_OVERVIEW.md)
3. [CONTRACT_POLICY.md](CONTRACT_POLICY.md)
4. [EXECUTOR_ADAPTER_CONTRACT.md](EXECUTOR_ADAPTER_CONTRACT.md)
5. [DEPLOYMENT_PLAN.md](DEPLOYMENT_PLAN.md)
6. [OFFICIAL_REFERENCE_NOTES.md](OFFICIAL_REFERENCE_NOTES.md)

[SYSTEM_BLUEPRINT.md](SYSTEM_BLUEPRINT.md) is the canonical top-level definition. The other documents refine specific parts of that model.

[OFFICIAL_REFERENCE_NOTES.md](OFFICIAL_REFERENCE_NOTES.md) collects vendor and platform references that support the feasibility of the current design without redefining the repository contracts.

## Governance

- [CONTRACT_POLICY.md](CONTRACT_POLICY.md)
- [DEVELOPMENT_PLAN_TEMPLATE.en.md](DEVELOPMENT_PLAN_TEMPLATE.en.md)
- [CONTRACT_FREEZE_TEMPLATE.en.md](CONTRACT_FREEZE_TEMPLATE.en.md)
- [CHANGE_REQUEST_POLICY.md](CHANGE_REQUEST_POLICY.md)
- [LANGUAGE_BOUNDARY_POLICY.md](LANGUAGE_BOUNDARY_POLICY.md)
- [STATUS_MODEL.md](STATUS_MODEL.md)
- [SECURITY_POLICY.md](SECURITY_POLICY.md)
- [BUDGET_POLICY.md](BUDGET_POLICY.md)
- [APPROVAL_CARD_SPEC.md](APPROVAL_CARD_SPEC.md)
- [GUI_EXCEPTION_POLICY.md](GUI_EXCEPTION_POLICY.md)

## Loop And Role Contracts

- [LOOP_SPEC.md](LOOP_SPEC.md)
- [STATE_STORE_SPEC.md](STATE_STORE_SPEC.md)
- [LIGHTWEIGHT_RUNTIME_PLAN.md](LIGHTWEIGHT_RUNTIME_PLAN.md)
- [EXECUTOR_ADAPTER_CONTRACT.md](EXECUTOR_ADAPTER_CONTRACT.md)
- [PLANNER_CONTRACT.en.md](PLANNER_CONTRACT.en.md)
- [BUILDER_CONTRACT.en.md](BUILDER_CONTRACT.en.md)
- [QA_CONTRACT.en.md](QA_CONTRACT.en.md)
- [REVIEW_CONTRACT.en.md](REVIEW_CONTRACT.en.md)
- [TASK_PACKET_TEMPLATE.en.md](TASK_PACKET_TEMPLATE.en.md)
- [HANDOFF_TEMPLATE.en.md](HANDOFF_TEMPLATE.en.md)
- [TRACEABILITY_SPEC.md](TRACEABILITY_SPEC.md)

## Archive And Metadata

- [ARTIFACT_LAYOUT_SPEC.md](ARTIFACT_LAYOUT_SPEC.md)
- [CHECKSUM_POLICY.md](CHECKSUM_POLICY.md)
- [JOB_MANIFEST_SCHEMA.md](JOB_MANIFEST_SCHEMA.md)

## GUI Runtime And Takeover

- [UBUNTU_GUI_RUNTIME_PLAN.md](UBUNTU_GUI_RUNTIME_PLAN.md)
- [TAKEOVER_FLOW.md](TAKEOVER_FLOW.md)
- [TAKEOVER_PACKET_TEMPLATE.en.md](TAKEOVER_PACKET_TEMPLATE.en.md)

## External Validation

- [OFFICIAL_REFERENCE_NOTES.md](OFFICIAL_REFERENCE_NOTES.md)

## Control-Plane Chinese Reference

[../codingclaw_ironclaw_revision_blueprint_cn.md](../codingclaw_ironclaw_revision_blueprint_cn.md) is a control-plane-only reference. It is not part of the English engineering delivery set under `docs/`.
